...
- `ROL_ANALISTA_VENTAS` existe y está integrado en la jerarquía de roles.
- El usuario del laboratorio puede activar el rol.
- El rol puede utilizar el warehouse, pero no administrarlo.
- El rol puede consultar las tablas de `CURATED` y las vistas de `MARTS`.
- Los future grants funcionan con objetos creados posteriormente.
- El rol no puede acceder a `STAGING`.
- El rol no puede insertar datos ni crear objetos.
- Las pruebas se han ejecutado con los roles secundarios desactivados.
- Los comandos de auditoría muestran únicamente los privilegios previstos.
...
1. La consulta que muestra el usuario actual.
2. La consulta del contexto con `ROL_ANALISTA_VENTAS` activo.
3. Una consulta correcta sobre `V_VENTAS_REGION`.
4. Una consulta correcta sobre `OBJETIVOS_REGION`.
5. Los cuatro errores esperados de las pruebas negativas.
6. El listado de grants del rol.
7. El listado de future grants del rol.
