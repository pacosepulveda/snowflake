...
## 18. Compatibilidad con Snowflake Trial y la versión actual

### Capacidades utilizadas

El ejercicio usa únicamente funciones y objetos disponibles en una cuenta trial normal:

- SQL Files de Snowsight.
- Roles del sistema.
- Virtual warehouses estándar.
- `ALTER WAREHOUSE` para suspender, reanudar y cambiar propiedades.
- Parámetros de sesión `QUERY_TAG` y `USE_CACHED_RESULT`.
- Funciones de contexto.
- `SHOW WAREHOUSES`.
- `LAST_QUERY_ID()`.
- `RESULT_SCAN()`.
- Funciones `QUERY_HISTORY_BY_SESSION` de `INFORMATION_SCHEMA`.
- Query History de Snowsight.

Ninguna de estas capacidades figura entre las limitaciones actuales documentadas para las cuentas trial.

### Edición

El ejercicio funciona en Standard y Enterprise. La edición Enterprise solicitada para el curso es totalmente válida, pero no se depende aquí de una característica exclusiva de Enterprise.

### Versión actual

Snowflake es SaaS y se actualiza de forma continua. La documentación oficial consultada en julio de 2026 mantiene vigentes:

- `ALTER WAREHOUSE ... SUSPEND | RESUME IF SUSPENDED`.
- Las propiedades `AUTO_SUSPEND` y `AUTO_RESUME`.
- `ALTER SESSION SET/UNSET QUERY_TAG` y `USE_CACHED_RESULT`.
- `LAST_QUERY_ID()`.
- `RESULT_SCAN()`.
- `QUERY_HISTORY_BY_SESSION()`.
- Los filtros por Query Tag en Query History de Snowsight.

El alumno puede registrar la versión concreta de su cuenta mediante:
...
Los datos son mínimos y las consultas son muy pequeñas. El consumo principal se produce al reanudar `WH_LAB_M2`.

Para un warehouse X-Small:

- Existe una facturación mínima de 60 segundos cada vez que se aprovisiona cómputo.
- Después del primer minuto, la facturación es por segundo.
- El laboratorio restaura `AUTO_SUSPEND = 60` y suspende manualmente al terminar.

El ejercicio debería consumir una fracción mínima del saldo disponible de una cuenta trial con 400 USD de crédito, siempre que el alumno no deje el warehouse ejecutándose después de terminar.

> Evita repetir innecesariamente el ciclo `SUSPEND → RESUME`, porque cada nuevo aprovisionamiento vuelve a aplicar el mínimo de 60 segundos.

---

## 19. Referencias oficiales utilizadas para la validación
